...
class (object):
    class RandomForest(object):
        ...
        def _bootstrapping(self, data_set, good_data, bad_data, n, percentage=0.05, strictly_percentage=False):
            """
            Return sample data and sample data label from a given data set by using bootstrapping method.
            :param data_set: data set, the last column is the data label
            :param good_data: the good data set, it it a part from data_set 
            :param bad_data: the bad data set, it it a part from data_set 
            :param n:  the number of rows in the data set
            :param percentage: how many percentage does a good data has to be in the final sample
            :param strictly_percentage: does the percentage above must be strictly followed?
            ...
            bad_data_length = len(bad_data)

            if not strictly_percentage:
                ...
                for i in repeat_times:
                    if i < good_data_number:
                        ...
                        random_pick_index = random.randint(0, bad_data_length - 1)
                        row = bad_data[random_pick_index]
                    sample_data.append(row[:-1])
                    sample_label.append(row[-1])
        ...
        def bootstrapping(self, data_set, percentage=0.05):
            ...
            good_data = self.get_specific_data(data_set)
            bad_data = self.get_specific_data(data_set, required_label=0)

            # Initialize the bootstrap data sets for each tree.
            for i in range(self.num_trees):
                # data_sample, data_label = self._bootstrapping(data_set, len(data_set))
                data_sample, data_label = self._bootstrapping(data_set, good_data, bad_data,
                                                              len(data_set), percentage, strictly_percentage=False)
                self.bootstraps_datasets.append(data_sample)
                self.bootstraps_labels.append(data_label)

        def get_specific_data(self, data_set, required_label=1):
            good_data = []
            for line in data_set:
                if required_label == line[-1]:
    ...
    def voting(self, X):
        return self.randomForest.voting([X])[0]

    def __init__(self, percentage=0.0, forest_size=10):
        """
        ...
        self.randomForest = self.RandomForest(forest_size)

        # Create the bootstrapping data sets
        self.randomForest.bootstrapping(XX, percentage)

        # Build trees in the forest
        self.randomForest.fitting()

        # Provide an unbiased error estimation of the random forest
        # based on out-of-bag (OOB) error estimate.
        y_truth = np.array(y, dtype=int)
        X = np.array(X, dtype=float)
        y_predicted = self.randomForest.voting(X)
