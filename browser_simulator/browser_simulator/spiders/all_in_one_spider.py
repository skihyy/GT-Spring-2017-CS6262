...
class TestSpiderSpider(scrapy.Spider):
    ...
    start_urls = ['https://zionma.github.io/simple.html']
    ...
    def parse(self, response):
        ...
        try:
            ...
            pass
        redirect_result = ''
        if 0 < len(tmp_redirect):
            redirect_result = self.html_redirect_handler(tmp_redirect, response.url)
            yield Request(redirect_result, callback=self.parse, headers=self.headers, meta=self.meta)
        else:
            # JS is embed in HTML
            # currently there is something wrong with PantomJS to stop redirect so the original spider will be used
            # for js redirect
            ...
            if not tmp_redirect:
                ...
                for file_src in js_file_location:
                    redirect_result = self.js_redirect_handler(file_src, response.url)
                    yield Request(redirect_result, callback=self.parse, headers=self.headers, meta=self.meta)
            else:
                redirect_result = self.js_redirect_handler(tmp_redirect, response.url)
                yield Request(redirect_result, callback=self.parse, headers=self.headers, meta=self.meta)
    ...
    def js_redirect_handler(self, redirect, cur_url):
    ...
    def html_redirect_handler(self, redirect, cur_url):
        # e.g. '0.5;url=http://helloworld.com'
        # e.g. '0.5;url=empty.exe'
        # after split, only need 'empty.exe'
